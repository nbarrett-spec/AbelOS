...
interface AuditReport {
  ...
  attributeCoverage: Record<string, { count: number; percentage: string }>;
  duplicates: {
    exactDuplicates: Array<{ name: string; count: number }>;
    similarDuplicates: Array<{ group: string; count: number }>;
    duplicateGroupCount: number;
  };
  priceAnalysis: {
    zeroPriceCount: number;
    zeroCostCount: number;
    costGreaterThanPrice: Array<{
      name: string;
      cost: number;
      basePrice: number;
    }>;
  ...
  nameQuality: {
    ...
    shortNames: Array<{ name: string; length: number }>;
    longNames: Array<{ name: string; length: number }>;
  };
  categoryAnalysis: {
  ...
  try {
    ...
    const allProducts = await prisma.product.findMany({
      select: {
        ...
        material: true,
        fireRating: true,
        imageUrl: true, // Check if this field exists in actual schema
      },
    });
    ...
    attributes.forEach((attr) => {
      ...
      allProducts.forEach((product) => {
        const value = product[attr as keyof typeof product];
