// Bookmark-friendly shortcut for Chad Zeh (PM) showing his full active job
// book. Chad covers many builders — unlike Brittney's or Thomas's pages
// this one intentionally does NOT filter by builderName. Every non-CLOSED /
// non-CANCELLED job where assignedPMId = Chad's Staff.id is included.
//
// If Chad ever narrows to a subset of builders, add a
// CHAD_BUILDER_PATTERNS list and mirror Brittney's / Thomas's ILIKE ANY
// filter.
//
...
// ── Page ──────────────────────────────────────────────────────────────────
export default async function ChadPage(): Promise<JSX.Element> {
  // Feature flag — default ON. Only hides when explicitly disabled.
  if (process.env.NEXT_PUBLIC_FEATURE_CHAD_PAGE === 'off') {
    return (
      <div className="max-w-3xl mx-auto p-8">
        <EmptyState
          title="This page is disabled"
          description="The /chad shortcut is turned off via feature flag."
        />
      </div>
    )
  }

  // ── Step 1: Look up Chad's Staff row ─────────────────────────────────
    ...
    return (
      <div className="max-w-4xl mx-auto p-8">
        <PageHeader
          eyebrow="PM Book"
          title="Chad's Book"
          description="Could not find Chad Zeh in the Staff directory."
        />
        <Card>
          <CardBody>
            <EmptyState
              ...
              description="No active staff row matches 'Chad Zeh'. He may not be set up yet, or his name is spelled differently in Staff."
              action={{
  ...
  // Chad covers many builders, so we pull every non-CLOSED / non-CANCELLED
  // job assigned to him. Mirrors Brittney's query shape minus the builder
  // ILIKE ANY clause.
  ...
  // Unique builder count — useful for Chad specifically since his book
  // spans many builders.
  const uniqueBuilders = new Set(
    jobs.map((j) => j.builderName).filter(Boolean)
  ).size
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-6 space-y-6">
      <PageHeader
        ...
        title="Chad's Book"
        description={`All active jobs assigned to ${staff.firstName}. Excludes CLOSED and CANCELLED. Not builder-filtered — Chad covers the full portfolio.`}
        crumbs={[
          { label: 'Ops', href: '/ops' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          value={totalJobs}
          accent="brand"
          subtitle={`${uniqueBuilders} builder${uniqueBuilders === 1 ? '' : 's'}`}
        />
        <KPICard
        ...
        <CardBody>
          {jobs.length === 0 ? (
            <EmptyState
              ...
              description="Chad has no open jobs right now. If this looks wrong, verify his Staff.id matches the assignedPMId on the jobs in InFlow / Bolt."
            />
          ) : (
            <Table>
              <TableHead>
