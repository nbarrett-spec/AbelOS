...
const URGENCY_COLORS: Record<string, string> = { URGENT: '#e74c3c', SOON: '#e67e22', UPCOMING: '#3498db' }
const STATUS_COLORS: Record<string, string> = { OPEN: '#3498db', IN_PROGRESS: '#e67e22', RESOLVED: '#27ae60', DENIED: '#e74c3c', ACTIVE: '#27ae60', EXPIRED: '#95a5a6' }
...
export default function WarrantyAutomationPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72', marginBottom: 4 }}>Warranty Automation</h1>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Expiring (90d)" value={Number(o.expiring90d || 0)} color="#e67e22" />
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Recent Claims</h3>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Soon (60d)" value={s.soon || 0} color="#e67e22" />
        <KPICard label="Upcoming (90d)" value={s.upcoming || 0} color="#3498db" />
      </div>

      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Warranties Expiring Soon</h3>
      ...
      {(data.recentlyExpired || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#e67e22' }}>Recently Expired — Extended Warranty Opportunity</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
                <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600 }}>Product</th>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Claims by Category</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
        ...
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Repeat Claimers</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
      ...
      {(data.monthly || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Monthly Claim Volume</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
          ...
          <tbody>
            {(data.builders || []).map((b: any, i: number) => (
              <tr key={b.id} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff' }}>
                ...
                <td style={{ padding: '10px 14px', textAlign: 'right', color: Number(b.expiringSoon) > 0 ? '#e67e22' : '#333', fontWeight: Number(b.expiringSoon) > 0 ? 700 : 400 }}>{Number(b.expiringSoon)}</td>
