...
export default function PredictiveAnalyticsPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72', marginBottom: 4 }}>AI Predictive Analytics</h1>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Weighted Pipeline" value={`$${Number(pl.weightedPipeline || 0).toLocaleString()}`} color="#E67E22" sub={`${pl.conversionRate || 0}% conv rate`} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <KPICard label="YTD Revenue" value={`$${Number(cp.ytdRevenue || 0).toLocaleString()}`} sub={`vs $${Number(cp.lastYearSamePeriod || 0).toLocaleString()} last year`} color="#1B4F72" />
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>6-Month Trajectory</h3>
            ...
            {(data.trajectory || []).map((t: any, i: number) => {
              ...
              return (
                <tr key={i} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff' }}>
                  ...
                  <td style={{ padding: '10px 14px' }}><MiniBar value={Number(t.revenue || 0)} max={maxRev} color="#1B4F72" /></td>
                </tr>
              )
            })}
          </tbody>
            ...
            {all.map((r: any, i: number) => {
              ...
              return (
                <tr key={i} style={{ borderTop: '1px solid #eee', background: isForecast ? '#f0f7ff' : (i % 2 ? '#fafafa' : '#fff') }}>
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 700, color: isForecast ? '#8e44ad' : '#1B4F72' }}>
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    <MiniBar value={Number(isForecast ? r.projected : r.revenue || 0)} max={maxRev} color={isForecast ? '#8e44ad' : '#1B4F72'} />
                  </td>
                </tr>
              )
            })}
...
function DemandView({ data }: { data: any }) {
  return (
    <div>
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Category Demand Trends</h3>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Top Products by Demand</h3>
      <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8f9fa' }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Day of Week</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {(data.dayOfWeek || []).map((d: any, i: number) => (
                  <tr key={i} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '8px 14px' }}><MiniBar value={Number(d.revenue || 0)} max={maxDow} color="#E67E22" height={14} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Monthly Seasonality</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
              ...
              <tbody>
                {(data.monthlyPattern || []).map((m: any, i: number) => (
                  <tr key={i} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '8px 14px' }}><MiniBar value={Number(m.revenue || 0)} max={maxMonth} color="#1B4F72" height={14} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {(data.hourOfDay || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>Hour of Day Activity</h3>
            ...
            {(data.hourOfDay || []).map((h: any) => {
              ...
              return (
                <div key={h.hour} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: '100%', background: '#1B4F72', borderRadius: '4px 4px 0 0', height: `${pct}%`, minHeight: Number(h.orders) > 0 ? 4 : 0 }} />
...
function CashFlowView({ data }: { data: any }) {
  return (
    <div>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#1B4F72' }}>8-Week Cash Flow Projection</h3>
