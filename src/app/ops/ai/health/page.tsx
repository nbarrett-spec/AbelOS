...
const SEVERITY_COLORS: Record<string, string> = { CRITICAL: '#e74c3c', HIGH: '#e74c3c', WARNING: '#e67e22', MEDIUM: '#e67e22', LOW: '#3498db', INFO: '#27ae60' }
const CATEGORY_COLORS: Record<string, string> = { COLLECTIONS: '#e74c3c', PURCHASING: '#8e44ad', SALES: '#E67E22', OPERATIONS: '#3498db' }
const GRADE_COLORS: Record<string, string> = { A: '#27ae60', B: '#2ecc71', C: '#f39c12', D: '#e67e22', F: '#e74c3c' }
...
function ComponentBar({ label, score, max }: { label: string; score: number; max: number }) {
  ...
  const color = pct >= 75 ? '#27ae60' : pct >= 50 ? '#f39c12' : pct >= 25 ? '#e67e22' : '#e74c3c'
  return (
...
export default function HealthMonitorPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72', marginBottom: 4 }}>Business Health Monitor</h1>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 24, marginBottom: 24 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16, color: '#1B4F72' }}>Health Components</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
        <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72' }}>Revenue</h4>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72' }}>Accounts Receivable</h4>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72' }}>Inventory</h4>
          ...
          <div style={{ fontSize: 13, color: '#e67e22' }}>Low stock: <strong>{Number(c.inventory?.lowStock || 0)}</strong></div>
        </div>

        <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72' }}>Operations</h4>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72' }}>Customer</h4>
...
function ScorecardsView({ data }: { data: any }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
      <CardSection title="Sales" color="#E67E22" items={[
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Warnings" value={s.warning || 0} color="#e67e22" />
              ...
              <tr key={i} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff' }}>
                ...
                <td style={{ padding: '10px 14px' }}>
                  <div style={{ height: 16, background: '#f0f0f0', borderRadius: 4, overflow: 'hidden' }}>
                    <div style={{ width: `${(Number(k.revenue || 0) / maxRev) * 100}%`, height: '100%', background: '#1B4F72', borderRadius: 4 }} />
                  </div>
                </td>
              </tr>
            ))}
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <KPICard label="Total Actions" value={s.total || 0} color="#1B4F72" />
        <KPICard label="Critical" value={s.critical || 0} color="#e74c3c" />
        <KPICard label="High" value={s.high || 0} color="#e67e22" />
        <KPICard label="Medium" value={s.medium || 0} color="#f39c12" />
      </div>
          ...
          <div key={i} style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                ...
                <Badge text={a.category} color={CATEGORY_COLORS[a.category] || '#1B4F72'} />
              </div>
