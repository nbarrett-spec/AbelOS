...
const PRIORITY_COLORS: Record<string, string> = { HIGH: '#e74c3c', MEDIUM: '#C6A24E', LOW: '#3498db' }
const STATUS_COLORS: Record<string, string> = { OVERLOADED: '#e74c3c', UNDERUTILIZED: '#D4B96A', BALANCED: '#27ae60' }
...
function UtilizationBar({ pct }: { pct: number }) {
  const color = pct > 90 ? '#e74c3c' : pct > 70 ? '#C6A24E' : pct > 40 ? '#27ae60' : '#3498db'
  return (
...
export default function SchedulingOptimizerPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400 }}>
      <h1 style={{ fontSize: 24, fontWeight: 600, color: '#0f2a3e', marginBottom: 4 }}>AI Scheduling Optimizer</h1>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        <KPICard label="Today's Jobs" value={Number(o.todayEntries || 0)} color="#0f2a3e" />
        <KPICard label="This Week" value={Number(o.thisWeekEntries || 0)} sub={`${o.completedThisWeek || 0} completed`} />
        <KPICard label="In Progress" value={Number(o.inProgress || 0)} color="#C6A24E" />
        <KPICard label="Overdue" value={Number(o.overdueEntries || 0)} color={Number(o.overdueEntries) > 0 ? '#e74c3c' : '#27ae60'} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Crew Status</h3>
                ...
                {(data.crewStatus || []).map((c: any, i: number) => (
                  <tr key={c.id} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff' }}>
                    ...
                    <td style={{ padding: '8px 14px', textAlign: 'right' }}>
                      {Number(c.activeNow) > 0 ? <Badge text="BUSY" color="#C6A24E" /> : <Badge text="IDLE" color="#27ae60" />}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Delivery Pipeline</h3>
          <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            {[
              ...
              { label: 'In Transit', value: d.inTransit, color: '#C6A24E' },
          ...
          {(data.entryTypes || []).length > 0 && (
            <>
              <h3 style={{ fontSize: 16, fontWeight: 600, margin: '20px 0 12px', color: '#0f2a3e' }}>Entry Types</h3>
              <div style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
...
function WorkloadView({ data }: { data: any }) {
  return (
    <div>
      <KPICard label="Avg Weekly Jobs/Crew" value={data.avgWeeklyJobsPerCrew || 0} color="#0f2a3e" />

      <h3 style={{ fontSize: 16, fontWeight: 600, margin: '20px 0 12px', color: '#0f2a3e' }}>Crew Workload Balance</h3>
      ...
      {(data.dailyDistribution || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Daily Distribution (This Week)</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Overdue" value={s.overdueCount || 0} color="#C6A24E" />
        <KPICard label="Unassigned" value={s.unassignedCount || 0} color="#D4B96A" />
      </div>
      ...
      {(data.overdue || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#C6A24E' }}>Overdue Entries</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: 24 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
      ...
      {(data.unassigned || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#D4B96A' }}>Unassigned Entries</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
                <th style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600 }}>Title</th>
...
function CapacityView({ data }: { data: any }) {
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        <KPICard label="Active Orders" value={Number(pj.activeOrders || 0)} color="#0f2a3e" />
        <KPICard label="Deliveries This Week" value={Number(pj.deliveriesThisWeek || 0)} color="#C6A24E" />
        <KPICard label="Next 2 Weeks" value={Number(pj.deliveriesNext2Weeks || 0)} color="#3498db" />
      </div>

      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>4-Week Capacity Outlook</h3>
      <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: 24 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr style={{ background: '#f8f9fa' }}>
      ...
      {(data.byType || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Capacity by Crew Type</h3>
            ...
            {(data.byType || []).map((t: any) => {
              ...
              return (
                <div key={t.crewType} style={{ background: '#fff', borderRadius: 10, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#0f2a3e', marginBottom: 12 }}>{t.crewType}</div>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <KPICard label="Suggestions" value={data.totalIssues || 0} color="#0f2a3e" />
