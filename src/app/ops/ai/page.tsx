...
export default function AIAssistantPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-white rounded-xl border overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#1B4F72] to-[#154360] px-6 py-4 text-white">
            ...
            >
              <div
                className={`max-w-md lg:max-w-lg px-4 py-3 rounded-lg ${
                  msg.role === 'user'
                    ? 'bg-[#1B4F72] text-white rounded-br-none'
          ...
          <div className="flex flex-wrap gap-2">
            {quickActions.map((action) => (
              <button
                ...
                disabled={loading}
                className="px-3 py-2 text-sm rounded-full bg-blue-50 text-[#1B4F72] border border-blue-200 hover:bg-blue-100 hover:border-blue-300 transition-all disabled:opacity-50"
              >
      ...
      <div className="bg-white border-t px-6 py-4">
        ...
        >
          <input
            ...
            disabled={loading}
            className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#1B4F72] disabled:bg-gray-100"
          />
          <button
            type="submit"
            disabled={loading || !input.trim()}
            className="px-4 py-2 bg-[#1B4F72] text-white rounded-lg hover:bg-[#154360] transition-colors disabled:bg-gray-300 font-medium"
          >
            Send
          </button>
        </form>
