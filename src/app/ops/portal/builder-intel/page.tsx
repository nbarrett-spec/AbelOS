...
const HEALTH_COLORS: Record<string, string> = { THRIVING: '#27ae60', HEALTHY: '#2ecc71', AT_RISK: '#C6A24E', CRITICAL: '#e74c3c', DORMANT: '#95a5a6', NEW: '#3498db' }
...
function ScoreBar({ score, max = 100 }: { score: number; max?: number }) {
  ...
  const color = pct >= 75 ? '#27ae60' : pct >= 50 ? '#2ecc71' : pct >= 25 ? '#C6A24E' : '#e74c3c'
  return (
...
export default function BuilderIntelPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, color: '#0f2a3e', marginBottom: 4 }}>Builder Intelligence Portal</h1>
...
function OverviewView({ data, onSelectBuilder }: { data: any; onSelectBuilder: (id: string) => void }) {
  return (
    <div>
      <KPICard label="Total Active Revenue" value={`$${Number(data.totalRevenue || 0).toLocaleString()}`} color="#0f2a3e" />

      <h3 style={{ fontSize: 16, fontWeight: 600, margin: '20px 0 12px', color: '#0f2a3e' }}>Revenue Concentration</h3>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 24 }}>
        {(data.revenueConcentration || []).map((r: any) => (
          <div key={r.tier} style={{ background: '#fff', borderRadius: 10, padding: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#0f2a3e' }}>{r.tier}</div>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, margin: '20px 0 12px', color: '#0f2a3e' }}>Account Health Distribution</h3>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Top Builders by Revenue</h3>
            ...
            {(data.topByRevenue || []).map((b: any, i: number) => (
              <tr key={b.id} onClick={() => onSelectBuilder(b.id)} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff', cursor: 'pointer' }}>
                <td style={{ padding: '10px 14px' }}>
                  <div style={{ fontWeight: 600, color: '#0f2a3e' }}>{b.companyName}</div>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="At Risk" value={s.atRisk || 0} color="#C6A24E" />
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Avg Margin/Builder" value={`$${Number(data.avgMarginPerBuilder || 0).toLocaleString()}`} color="#0f2a3e" />
      </div>
      <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr style={{ background: '#f8f9fa' }}>
          ...
          <tbody>
            {(data.builders || []).slice(0, 40).map((b: any, i: number) => (
              <tr key={b.id} style={{ borderTop: '1px solid #eee', background: i % 2 ? '#fafafa' : '#fff' }}>
                ...
                <td style={{ padding: '10px 14px', textAlign: 'right' }}>{Number(b.orderCount)}</td>
                <td style={{ padding: '10px 14px' }}>
                  <Badge text={b.costToServe} color={b.costToServe === 'LOW' ? '#27ae60' : b.costToServe === 'HIGH' ? '#e74c3c' : '#C6A24E'} />
                </td>
...
function AffinityView({ data }: { data: any }) {
  return (
    <div>
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Category Co-Purchase Patterns</h3>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Product Pair Affinity</h3>
    ...
    <div>
      <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 22, color: '#0f2a3e' }}>{b.companyName}</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        <KPICard label="Total Revenue" value={`$${Number(os.totalSpend || 0).toLocaleString()}`} color="#0f2a3e" />
        ...
        <KPICard label="Largest Order" value={`$${Number(os.largestOrder || 0).toLocaleString()}`} color="#C6A24E" />
      </div>

      {(data.topProducts || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Top Products Purchased</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: 24 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
      ...
      {(data.spendTrend || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Monthly Spend Trend</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', marginBottom: 24 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
                  ...
                  <tr key={i} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '8px 14px' }}>
                      <div style={{ height: 14, background: '#f0f0f0', borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ width: `${(Number(t.revenue || 0) / maxR) * 100}%`, height: '100%', background: '#0f2a3e', borderRadius: 4 }} />
                      </div>
                    </td>
                  </tr>
                )) })()}
      ...
      {(data.customPricing || []).length > 0 && (
        <>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#0f2a3e' }}>Custom Pricing</h3>
          <div style={{ background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ background: '#f8f9fa' }}>
                <th style={{ padding: '8px 14px', textAlign: 'left', fontWeight: 600 }}>Product</th>
              ...
              <tbody>
                {(data.customPricing || []).map((p: any, i: number) => (
                  <tr key={p.id} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '8px 14px', textAlign: 'right', fontWeight: 600, color: '#C6A24E' }}>${Number(p.customPrice || 0).toFixed(2)}</td>
