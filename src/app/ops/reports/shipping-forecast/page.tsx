      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <a href="/ops/reports" className="text-sm text-gray-400 hover:text-[#1B4F72] transition">&larr; Reports</a>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 text-sm font-medium rounded-md transition ${
                  days === opt.value ? 'bg-white shadow text-[#1B4F72]' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {opt.label}
              </button>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
            <div className="bg-white rounded-xl border p-5">
              ...
              <p className="text-3xl font-bold text-[#1B4F72] mt-1">{fmt(data.meta.orderCount)}</p>
              <p className="text-xs text-gray-400 mt-1">Next {days} days</p>
            </div>
            <div className="bg-white rounded-xl border p-5">
              <p className="text-sm text-gray-500">Total Revenue</p>
              <p className="text-3xl font-bold text-[#E67E22] mt-1">{fmtCurrency(totalRevenue)}</p>
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-1.5 text-sm font-medium rounded-md transition ${
                  tab === t.key ? 'bg-white shadow text-[#1B4F72]' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {t.label}
              </button>
              ...
              {Object.entries(dateGroups).map(([date, rows]) => {
                ...
                return (
                  <div key={date} className="bg-white rounded-xl border overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-[#1B4F72] to-[#2980B9]">
                      ...
                      <tbody>
                        {rows.map((r, i) => (
                          <tr key={r.orderNumber} className={i % 2 ? 'bg-gray-50/50' : ''}>
                            <td className="px-5 py-2.5 font-medium text-[#1B4F72]">{r.orderNumber}</td>
          ...
          {tab === 'orders' && (
            <div className="bg-white rounded-xl border overflow-hidden">
              <table className="w-full text-sm">
                  ...
                  {data.orders.map((o, i) => (
                    <tr key={o.orderNumber} className={`${i % 2 ? 'bg-gray-50/50' : ''} hover:bg-blue-50/30 transition`}>
                      <td className="px-5 py-2.5">
                        <a href={`/ops/orders/${o.orderNumber}`} className="font-medium text-[#1B4F72] hover:underline">
                          {o.orderNumber}
                        </a>
                      </td>
                ...
                <tfoot>
                  <tr className="bg-[#1B4F72]/5 font-bold">
                    <td className="px-5 py-3 text-[#1B4F72]" colSpan={5}>Grand Total</td>
                    ...
                    <td className="px-5 py-3 text-right text-[#1B4F72]">{fmtCurrency(totalRevenue)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
                  ...
                  <tbody>
                    {data.bomTotals.map((b, i) => (
                      <tr key={b.component} className={i % 2 ? 'bg-gray-50/50' : ''}>
                        ...
                        <td className="px-5 py-2.5 text-right font-semibold text-[#1B4F72]">{fmt(b.totalNeeded)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {data.adtDoors.map((d, i) => (
                      <tr key={`${d.orderNumber}-${d.sku}-${i}`} className={i % 2 ? 'bg-gray-50/50' : ''}>
                        <td className="px-5 py-2.5 font-medium text-[#1B4F72]">{d.orderNumber}</td>
