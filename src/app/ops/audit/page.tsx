...
const severityColors: Record<string, { bg: string; text: string; dot: string }> = {
  INFO: { bg: '#EBF5FB', text: '#0f2a3e', dot: '#3498DB' },
  WARN: { bg: '#FEF9E7', text: '#7D6608', dot: '#D4B96A' },
  CRITICAL: { bg: '#FDEDEC', text: '#922B21', dot: '#E74C3C' },
}

const actionColors: Record<string, { bg: string; text: string }> = {
  CREATE: { bg: '#E8F8F5', text: '#1E8449' },
  UPDATE: { bg: '#EBF5FB', text: '#0f2a3e' },
  ...
  STATUS_CHANGE: { bg: '#EBF5FB', text: '#0f2a3e' },
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#0f2a3e', margin: 0 }}>Audit Trail</h1>
          <p style={{ color: '#6b7280', margin: '4px 0 0', fontSize: '14px' }}>
            Every data change tracked with who, what, when, and why
          </p>
        </div>
        <button onClick={() => fetchLogs(offset)} style={{ background: '#0f2a3e', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer', fontSize: '14px' }}>
          Refresh
        </button>
      </div>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          {[
            { label: 'Total Events', value: stats.totalLogs, color: '#0f2a3e', icon: '\uD83D\uDCCB' },
            { label: 'Critical', value: stats.criticalCount, color: '#E74C3C', icon: '\uD83D\uDD34' },
            { label: 'Warnings', value: stats.warnCount, color: '#D4B96A', icon: '\u26A0\uFE0F' },
              ...
              {loading ? (
                ...
                return (
                  <tr key={log.id} style={{ borderBottom: '1px solid #f3f4f6', cursor: 'pointer' }} onClick={() => setSelected(log)}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <button onClick={(e) => { e.stopPropagation(); setSelected(log) }} style={{ background: '#EBF5FB', color: '#0f2a3e', border: 'none', borderRadius: '6px', padding: '4px 10px', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>View</button>
                    </td>
                  </tr>
                )
              })}
        ...
        {total > limit && (
          <div style={{ padding: '12px 16px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => fetchLogs(Math.max(0, offset - limit))} disabled={offset === 0} style={{ padding: '6px 14px', background: offset === 0 ? '#e5e7eb' : '#0f2a3e', color: offset === 0 ? '#9ca3af' : 'white', border: 'none', borderRadius: '6px', fontSize: '13px', cursor: offset === 0 ? 'default' : 'pointer' }}>\u2190 Prev</button>
              <span style={{ padding: '6px 10px', fontSize: '13px', color: '#374151' }}>Page {page} / {totalPages}</span>
              <button onClick={() => fetchLogs(offset + limit)} disabled={offset + limit >= total} style={{ padding: '6px 14px', background: offset + limit >= total ? '#e5e7eb' : '#0f2a3e', color: offset + limit >= total ? '#9ca3af' : 'white', border: 'none', borderRadius: '6px', fontSize: '13px', cursor: offset + limit >= total ? 'default' : 'pointer' }}>Next \u2192</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '12px', padding: '24px', maxWidth: '600px', width: '90%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 25px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#0f2a3e', margin: 0 }}>Audit Event Detail</h2>
            ...
            <button onClick={() => setSelected(null)} style={{ marginTop: '16px', width: '100%', padding: '10px', background: '#0f2a3e', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Close</button>
          </div>
        </div>
      )}
    </div>
