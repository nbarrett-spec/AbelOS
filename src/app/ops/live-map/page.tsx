...
const JOB_STATUSES = [
  { key: 'ALL', label: 'All', color: '#1B4F72' },
  ...
  { key: 'LOADED', label: 'Loaded', color: '#E67E22' },
...
const TRUCK_STATUSES: Record<string, { label: string; color: string }> = {
  EN_ROUTE: { label: 'En Route', color: '#E67E22' },
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="pt-2 space-y-2">
        <Link
          ...
          className="block w-full text-center px-4 py-2.5 rounded-xl bg-[#1B4F72] text-white text-sm font-semibold hover:bg-[#154360] transition-colors"
        >
          Open Job Profile →
        </Link>
        <Link
          ...
          <div className="space-y-1.5">
            {truck.crew.members.map((m, i) => (
              <div key={i} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800/50 rounded-lg px-3 py-2">
                <div className="w-7 h-7 rounded-full bg-[#1B4F72] flex items-center justify-center text-xs font-bold text-white">
