...
export default function CashFlowOptimizerPage() {
  ...
  if (loading) {
    return (
      <div style={{ padding: 32, textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#1B4F72', marginBottom: 8 }}>Loading Cash Flow Intelligence...</h2>
  ...
  if (error) {
    return (
      <div style={{ padding: 32, textAlign: 'center' }}>
        ...
        <button onClick={fetchData} style={{ padding: '8px 24px', background: '#1B4F72', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Retry</button>
      </div>
    )
  }
  ...
  const healthColor = healthScore >= 75 ? '#27ae60' : healthScore >= 50 ? '#E67E22' : '#e74c3c'
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#1B4F72', margin: 0 }}>💸 Cash Flow Command Center</h1>
          <p style={{ color: '#666', margin: '4px 0 0' }}>Collections intelligence, payment optimization, working capital forecasting</p>
        </div>
        <button onClick={fetchData} style={{ padding: '10px 20px', background: '#1B4F72', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
          🔄 Refresh
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 14, marginBottom: 24 }}>
        ...
        <KPICard label="Working Capital" value={formatCurrency(position.workingCapital)} icon="💰" color="#1B4F72" />
        ...
        <KPICard label="DSO" value={`${metrics.dso} days`} subtitle={`DPO: ${metrics.dpo}d`} icon="⏱️" color="#E67E22" />
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#1B4F72', fontSize: 16 }}>💰 Working Capital Breakdown</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ borderTop: '2px dashed #1B4F72', paddingTop: 10, display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontWeight: 700, color: '#1B4F72' }}>Net Working Capital</span>
      ...
      {/* AR Aging */}
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#1B4F72', fontSize: 16 }}>📊 Receivables Aging</h3>
        {collectionSummary.invoiceCount === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {Object.entries(aging).map(([bucket, data]) => {
              const colors: Record<string, string> = { current: '#27ae60', '1-30': '#f39c12', '31-60': '#E67E22', '61-90': '#e74c3c', '90+': '#c0392b' }
              return (
      ...
      {/* Key Metrics */}
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#1B4F72', fontSize: 16 }}>⚙️ Financial Health Metrics</h3>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        ...
        {wcRecs.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {wcRecs.map((rec, i) => {
              const priorityColors: Record<string, string> = { HIGH: '#e74c3c', MEDIUM: '#E67E22', LOW: '#27ae60' }
              return (
                <div key={i} style={{ padding: 14, background: '#fff8f0', borderRadius: 8, border: '1px solid #fde8d0' }}>
                  ...
                  {rec.impact !== 0 && (
                    <div style={{ fontSize: 12, color: '#E67E22', fontWeight: 600, marginTop: 4 }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e8e8e8', textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72' }}>{formatCurrency(summary.totalAccountsReceivable)}</div>
        ...
        <div style={{ background: '#fff3cd', borderRadius: 12, padding: 16, border: '1px solid #ffc107', textAlign: 'center' }}>
          <div style={{ fontSize: 24, fontWeight: 700, color: '#E67E22' }}>{summary.highCount}</div>
          ...
          <tbody>
            {actions.map((a, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ textAlign: 'center', padding: '10px 8px', color: a.daysOverdue > 30 ? '#e74c3c' : '#E67E22' }}>{a.daysOverdue}d</td>
...
function PaymentTermsTab({ recommendations, summary }: { recommendations: PaymentTermRec[]; summary: any }) {
  if (recommendations.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: 60 }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>📋</div>
        <h3 style={{ color: '#1B4F72', marginBottom: 8 }}>Payment Term Analysis</h3>
        <p style={{ color: '#888', maxWidth: 400, margin: '0 auto' }}>
    ...
    <div>
      {summary && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e8e8e8', textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72' }}>{recommendations.length}</div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e8e8e8', textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#E67E22' }}>{(recommendations.reduce((s, r) => s + r.onTimeRate, 0) / recommendations.length * 100).toFixed(0)}%</div>
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8', marginBottom: 20 }}>
        <h3 style={{ margin: '0 0 16px', color: '#1B4F72', fontSize: 16 }}>📈 90-Day Cash Flow Forecast</h3>
        {forecast.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 40, color: '#999' }}>
                ...
                return (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }} title={`Day ${day.day}: ${formatCurrency(day.runningBalance)}`}>
                    <div style={{
                      ...
                      background: isNeg ? '#e74c3c' : day.runningBalance > position.workingCapital * 0.5 ? '#27ae60' : '#E67E22',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72', fontSize: 14 }}>📊 Current Position</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72', fontSize: 14 }}>🔄 Cash Conversion</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #eee', paddingTop: 8 }}><span style={{ color: '#1B4F72', fontSize: 13, fontWeight: 600 }}>Cash Cycle</span><span style={{ fontWeight: 700, color: '#1B4F72' }}>{metrics.ccc} days</span></div>
          </div>
        </div>
        <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
          <h4 style={{ margin: '0 0 12px', color: '#1B4F72', fontSize: 14 }}>📐 Ratios</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#666', fontSize: 13 }}>Current Ratio</span><span style={{ fontWeight: 700, color: metrics.currentRatio >= 2 ? '#27ae60' : '#E67E22' }}>{metrics.currentRatio.toFixed(2)}x</span></div>
...
function MetricBox({ label, value, unit, target, good }: { label: string; value: string; unit: string; target: string; good: boolean }) {
  return (
    <div style={{ padding: 12, background: good ? '#f0fdf4' : '#fef9f0', borderRadius: 8, border: `1px solid ${good ? '#d1fae5' : '#fde8d0'}` }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 700, color: good ? '#27ae60' : '#E67E22' }}>{value}<span style={{ fontSize: 12, fontWeight: 400 }}> {unit}</span></div>
...
function UrgencyBadge({ urgency }: { urgency: string }) {
  const config: Record<string, { bg: string; color: string }> = {
    ...
    HIGH: { bg: '#fef3cd', color: '#E67E22' },
    MEDIUM: { bg: '#fff3e0', color: '#f39c12' },
