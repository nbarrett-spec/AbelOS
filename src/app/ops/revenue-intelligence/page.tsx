  ...
  if (loading) {
    return (
      <div style={{ padding: 32, textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#0f2a3e', marginBottom: 8 }}>Loading Revenue Intelligence...</h2>
  ...
  if (error) {
    return (
      <div style={{ padding: 32, textAlign: 'center' }}>
        ...
        <button onClick={fetchData} style={{ padding: '8px 24px', background: '#0f2a3e', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
          Retry
        </button>
      </div>
    )
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#0f2a3e', margin: 0 }}>
        ...
        <button
          onClick={fetchData}
          style={{ padding: '10px 20px', background: '#0f2a3e', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14 }}
        >
          🔄 Refresh Data
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <KPICard label="Active Builders" value={builderSummary.totalActiveBuilders.toString()} icon="👷" color="#0f2a3e" />
        <KPICard label="Avg Margin" value={`${avgMarginPct.toFixed(1)}%`} icon="📊" color="#C6A24E" />
        <KPICard label="Elite Builders" value={`${platinumCount + goldCount}`} subtitle={`${platinumCount} Platinum · ${goldCount} Gold`} icon="⭐" color="#D4B96A" />
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#0f2a3e', fontSize: 16 }}>🏆 Builder Segments</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {(['PLATINUM', 'GOLD', 'SILVER', 'STANDARD'] as const).map(seg => {
            const data = segments[seg] || { count: 0, totalRevenue: 0, avgLTV: 0 }
            const colors: Record<string, string> = { PLATINUM: '#8e44ad', GOLD: '#D4B96A', SILVER: '#95a5a6', STANDARD: '#3498db' }
      ...
      {/* Top Builders */}
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#0f2a3e', fontSize: 16 }}>🏅 Top 10 Builders by LTV</h3>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        ...
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #eee' }}>
          <h4 style={{ margin: '0 0 12px', color: '#0f2a3e', fontSize: 14 }}>🏷️ Active Pricing Rules</h4>
...
function PricingTab({ rules, stats, recommendations }: { rules: PricingRule[]; stats: any; recommendations: PricingRecommendation[] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      {/* Active Rules */}
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#0f2a3e', fontSize: 16 }}>🏷️ Active Pricing Rules</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {/* Stats */}
        <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
          <h3 style={{ margin: '0 0 16px', color: '#0f2a3e', fontSize: 16 }}>📊 Optimization Performance (30 Days)</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ padding: 12, background: '#f0f8ff', borderRadius: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#0f2a3e' }}>{stats.quotesOptimized}</div>
            ...
            <div style={{ padding: 12, background: '#fef9f0', borderRadius: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#C6A24E' }}>{formatCurrency(stats.totalRevenueImpact)}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {recommendations.map((rec, i) => (
                <div key={i} style={{ padding: 12, background: '#fff8f0', borderRadius: 8, border: '1px solid #fde8d0' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#C6A24E' }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e8e8e8', textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#0f2a3e' }}>{marginAnalysis.length}</div>
          <div style={{ fontSize: 12, color: '#888' }}>Product Categories</div>
        </div>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e8e8e8', textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: avgMargin >= 25 ? '#27ae60' : '#C6A24E' }}>{avgMargin.toFixed(1)}%</div>
      ...
      {/* Margin Bars */}
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, border: '1px solid #e8e8e8' }}>
        <h3 style={{ margin: '0 0 16px', color: '#0f2a3e', fontSize: 16 }}>📊 Margin by Product Category</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {sortedMargins.map(m => {
            const pct = m.marginPercent
            const barColor = pct >= 30 ? '#27ae60' : pct >= 20 ? '#C6A24E' : '#e74c3c'
            return (
              <div key={m.category}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
...
function SegmentBadge({ segment }: { segment: string }) {
  const config: Record<string, { bg: string; color: string; icon: string }> = {
    ...
    GOLD: { bg: '#fff8e1', color: '#D4B96A', icon: '🥇' },
...
function ScoreBadge({ score }: { score: number }) {
  const color = score >= 70 ? '#27ae60' : score >= 40 ? '#C6A24E' : '#e74c3c'
  return (
...
function RiskBadge({ risk }: { risk: string }) {
  const config: Record<string, { bg: string; color: string; label: string }> = {
    ...
    MEDIUM: { bg: '#fef3cd', color: '#D4B96A', label: '🟡 Med' },
