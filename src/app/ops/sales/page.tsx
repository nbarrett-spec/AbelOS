      ...
      <div className="bg-[#1e3a5f] text-white px-8 py-8">
        <div className="flex items-start justify-between">
          ...
          <button
            ...
            className="bg-[#E67E22] hover:bg-[#d46711] text-white px-8 py-3 rounded-lg font-bold text-base transition-colors transform hover:scale-105"
          >
            + New Deal
          </button>
        </div>
                  ...
                  return (
                    <Link key={deal.id} href={`/ops/sales/deals/${deal.id}`}>
                      <div className={`bg-white rounded-lg p-4 border ${isUrgent ? 'border-red-300 ring-1 ring-red-200' : 'border-gray-200'} hover:shadow-md transition-all cursor-pointer`}>
                        ...
                        <div className="flex items-center justify-between">
                          <p className="text-lg font-bold text-[#E67E22]">{formatCurrency(deal.dealValue)}</p>
                ...
                return (
                  <Link key={deal.id} href={`/ops/sales/deals/${deal.id}`}>
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow cursor-pointer h-full">
                      ...
                      <p className="text-lg font-bold text-[#E67E22] mb-2">{formatCurrency(deal.dealValue)}</p>
                          ...
                          stageDeals.map((deal) => (
                            <Link key={deal.id} href={`/ops/sales/deals/${deal.id}`}>
                              <div className="bg-white border border-gray-200 rounded-lg p-3 cursor-pointer hover:shadow-md transition-shadow">
                                ...
                                <p className="font-bold text-sm text-[#e67e22] mt-2">
                  ...
                  {sortedDeals.map((deal) => (
                    <Link key={deal.id} href={`/ops/sales/deals/${deal.id}`}>
                      <tr className="hover:bg-gray-50 cursor-pointer transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-[#E67E22]">{formatCurrency(deal.dealValue)}</td>
                  ...
                  <div key={index} className="p-3 rounded-lg bg-gray-50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="text-sm font-bold text-[#E67E22]">{formatCurrency(rep.pipelineValue)}</p>
                        <p className="text-xs text-gray-500">pipeline</p>
                      </div>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className="bg-gradient-to-r from-[#1e3a5f] to-[#E67E22] h-2 rounded-full"
                        style={{
                          width: `${
