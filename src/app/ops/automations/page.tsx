  ...
  if (loading) {
    return (
      <div style={{ padding: 32, textAlign: 'center' }}>
        <div style={{ fontSize: 18, color: '#1B4F72' }}>Loading automations...</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#1B4F72', margin: 0 }}>Automations & AI Tasks</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'Total Rules', value: rules.length, color: '#1B4F72' },
          ...
          { label: 'Total Executions', value: rules.reduce((s, r) => s + (r.runCount || 0), 0), color: '#E67E22' },
        ].map((stat, i) => (
          <div key={i} style={{
      ...
      {activeTab === 'rules' && (
        <div>
          ...
          {showCreateForm && (
            <div style={{
              background: '#f8f9fa', border: '2px solid #1B4F72', borderRadius: 12,
              padding: 24, marginBottom: 24,
            }}>
              <h3 style={{ margin: '0 0 16px', color: '#1B4F72' }}>Create New Automation Rule</h3>
                  ...
                  {ROLE_OPTIONS.map(role => (
                    <button key={role} onClick={() => toggleFormRole(role)}
                      style={{
                        ...
                        border: formRoles.includes(role) ? '2px solid #1B4F72' : '1px solid #ccc',
                        background: formRoles.includes(role) ? '#1B4F72' : '#fff',
                    ...
                    return (
                      <button key={action.value} onClick={() => toggleFormAction(action.value)}
                        style={{
                          ...
                          border: selected ? '2px solid #E67E22' : '1px solid #ddd',
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <button
                  ...
                  style={{
                    background: (!formName || !formTrigger) ? '#ccc' : '#1B4F72', color: '#fff',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                        <span style={{ fontSize: 16, fontWeight: 700, color: '#1B4F72' }}>{rule.name}</span>
                        <span style={{
                      ...
                      <div style={{ display: 'flex', gap: 4, marginTop: 8, flexWrap: 'wrap' }}>
                        {(rule.roles || []).map((r: string) => (
                          <span key={r} style={{
                            padding: '2px 8px', background: '#EBF5FB', color: '#1B4F72',
                      ...
                      {rule.actions && (rule.actions as any[]).length > 0 && (
                        <div style={{ display: 'flex', gap: 4, marginTop: 6, flexWrap: 'wrap' }}>
                          {(rule.actions as any[]).map((a: any, i: number) => {
                            ...
                            return (
                              <span key={i} style={{
                                padding: '2px 8px', background: '#FFF3E6', color: '#E67E22',
                                borderRadius: 10, fontSize: 10, fontWeight: 500,
                              }}>
          ...
          <div style={{ marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button onClick={() => setTemplateFilter('')}
              style={{
                ...
                border: !templateFilter ? '2px solid #1B4F72' : '1px solid #ccc',
                background: !templateFilter ? '#1B4F72' : '#fff',
            ...
            {templateCategories.map(cat => (
              <button key={cat} onClick={() => setTemplateFilter(cat)}
                style={{
                  padding: '6px 14px', borderRadius: 16, fontSize: 12, cursor: 'pointer', fontWeight: 600,
                  border: templateFilter === cat ? '2px solid #1B4F72' : '1px solid #ccc',
                  background: templateFilter === cat ? '#1B4F72' : '#fff',
                ...
                <div key={idx} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 700, color: '#1B4F72', marginTop: 4 }}>{template.name}</div>
                    </div>
                    {installed ? (
                      ...
                      <button onClick={() => installTemplate(template)} disabled={saving}
                        style={{
                          padding: '6px 16px', background: '#E67E22', color: '#fff', border: 'none',
                  ...
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
                    {template.roles.map(r => (
                      <span key={r} style={{
                        padding: '2px 6px', background: '#EBF5FB', color: '#1B4F72',
                  ...
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {template.actions.map((a, i) => {
                      ...
                      return (
                        <span key={i} style={{
                          padding: '2px 6px', background: '#FFF3E6', color: '#E67E22',
                          borderRadius: 8, fontSize: 9, fontWeight: 500,
                        }}>
                          {opt?.icon} {opt?.label || a.type}
                        </span>
                ...
                <tbody>
                  {logs.map(log => (
                    <tr key={log.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                      ...
                      <td style={{ padding: '10px 16px', fontWeight: 600, color: '#1B4F72' }}>{log.ruleName || '—'}</td>
