  ...
  if (!data) {
    return (
      <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 20px' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1B4F72', marginBottom: 8 }}>🖨️ Print Job Packet</h1>
  ...
  return (
    <>
      ...
      <div className="no-print" style={{ background: '#F9FAFB', borderBottom: '1px solid #E5E7EB', padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <span style={{ fontWeight: 700, color: '#1B4F72' }}>{data.job.jobNumber}</span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={handlePrint}
            style={{ padding: '8px 16px', background: '#1B4F72', color: 'white', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            🖨️ Print Job Packet
          </button>
        </div>
      </div>
      ...
      <div ref={printRef} id="job-packet">
        <style>{`
          ...
          .section-header { background: #1B4F72; color: white; padding: 8px 14px; font-size: 14px; font-weight: 700; margin: 0; }
          .sub-header { background: #E67E22; color: white; padding: 6px 14px; font-size: 12px; font-weight: 700; margin: 0; }
        `}</style>
        ...
        <div style={{ padding: 20 }}>
          {/* Abel header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1B4F72', paddingBottom: 12, marginBottom: 16 }}>
            <div>
              <h1 style={{ fontSize: 26, fontWeight: 800, color: '#1B4F72', margin: 0 }}>ABEL LUMBER</h1>
              <p style={{ fontSize: 11, color: '#6B7280', margin: 0 }}>Manufacturing Job Packet</p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#E67E22' }}>{data.job.jobNumber}</div>
        ...
        <div className="page-break" style={{ padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '3px solid #1B4F72', paddingBottom: 8, marginBottom: 12 }}>
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: '#1B4F72', margin: 0 }}>PICK LIST</h2>
              <p style={{ fontSize: 11, color: '#6B7280', margin: 0 }}>Sorted by zone → bin for warehouse walk path</p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#E67E22' }}>{data.job.jobNumber}</div>
        ...
        {data.assemblyGroups.map((group, gIdx) => (
          <div key={gIdx} className="page-break" style={{ padding: 20 }}>
            {/* Unit header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1B4F72', paddingBottom: 8, marginBottom: 12 }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 800, color: '#1B4F72', margin: 0 }}>BUILD SHEET</h2>
                <p style={{ fontSize: 11, color: '#6B7280', margin: 0 }}>Unit {gIdx + 1} of {data.assemblyGroups.length}</p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#E67E22' }}>{data.job.jobNumber}</div>
                <div style={{ fontSize: 11, color: '#6B7280' }}>{data.job.builderName}</div>
              </div>
            </div>

            {/* Door identity */}
            <div style={{ border: '2px solid #1B4F72', borderRadius: 4, marginBottom: 16, overflow: 'hidden' }}>
              <div style={{ background: '#1B4F72', color: 'white', padding: '8px 14px', fontSize: 16, fontWeight: 700 }}>
                {group.parent.name}
              </div>
        ...
        <div className="page-break" style={{ padding: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1B4F72', paddingBottom: 8, marginBottom: 12 }}>
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: '#1B4F72', margin: 0 }}>DELIVERY & INSTALLATION</h2>
