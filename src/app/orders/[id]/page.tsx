    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-5xl mx-auto px-4 py-16 text-center">
          ...
          <Link href="/dashboard/orders" className="text-[#C6A24E] hover:text-[#A8882A] font-medium">← Back to Orders</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          <Link href="/dashboard" className="hover:text-[#0f2a3e]">Dashboard</Link>
          <span>/</span>
          <Link href="/dashboard/orders" className="hover:text-[#0f2a3e]">Orders</Link>
          <span>/</span>
        ...
        <div className="bg-white rounded-2xl border p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            ...
            <div className="sm:text-right">
              <p className="text-2xl sm:text-3xl font-bold text-[#0f2a3e]">${order.total?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              {order.deliveryDate && (
          ...
          <div className="flex flex-wrap gap-3 mt-5 pt-5 border-t">
            <button
              ...
              className="px-4 py-2 bg-[#0f2a3e] text-white text-sm font-medium rounded-lg hover:bg-[#1a2f4e] transition"
            >
              Contact Abel Lumber
            </button>
            <button
              ...
              return (
                <div key={step.key} className="flex items-start gap-4 mb-6 last:mb-0">
                  ...
                  <div className={`pt-1 ${isComplete ? '' : 'opacity-40'}`}>
                    <p className={`font-semibold ${isCurrent ? 'text-[#C6A24E]' : isComplete ? 'text-gray-900' : 'text-gray-400'}`}>
                      {step.label}
                      {isCurrent && <span className="ml-2 text-xs bg-[#C6A24E] text-white px-2 py-0.5 rounded-full">Current</span>}
                    </p>
        ...
        <div className="bg-white rounded-2xl border overflow-hidden mb-6">
          ...
          <div className="px-6 py-4 border-t bg-gray-50 flex justify-between items-center">
            ...
            <span className="text-xl font-bold text-[#0f2a3e]">${order.total?.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          </div>
        </div>
        ...
        {order.invoiceId && (
          <div className="bg-white rounded-2xl border p-6 mb-6">
            ...
            <Link href="/dashboard/invoices" className="text-[#C6A24E] hover:text-[#A8882A] font-medium text-sm">
              View Invoice →
            </Link>
          </div>
        )}
