...
export default function WarrantyPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'CLOSED':
        return 'bg-surface-muted text-fg border border-border-strong';
      default:
        return 'bg-surface-muted text-fg border border-border-strong';
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {activePolicies.length > 0 ? (
                activePolicies.map((policy) => (
                  <div key={policy.id} className="border border-border rounded-lg p-4 hover:shadow-md transition-shadow">
                    ...
                    <p className="text-sm text-fg-muted mt-3">{policy.description}</p>
                    <p className="text-sm font-semibold text-fg mt-3">Duration: {policy.durationMonths} months</p>
                  </div>
                ))
              ) : (
                <p className="text-fg-muted col-span-full">No active policies</p>
              )}
            </div>
          )}
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-surface-muted border-b border-border">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Claim #</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Type</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Subject</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Status</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Priority</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Date</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-fg">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {claims.map((claim) => (
                    <tr key={claim.id} className="hover:bg-surface-muted transition-colors cursor-pointer">
                      <td className="px-6 py-4 text-sm font-semibold text-fg">{claim.claimNumber}</td>
                      <td className="px-6 py-4 text-sm">
                        <span className="text-xl">{getTypeIcon(claim.type)}</span>
                        <span className="text-fg-muted ml-2">{claim.type}</span>
                      </td>
                      <td className="px-6 py-4 text-sm text-fg-muted">{claim.subject}</td>
                      ...
                      <td className="px-6 py-4 text-sm text-fg-muted">{formatDate(claim.createdAt)}</td>
                      <td className="px-6 py-4">
                        <button
          ...
          <div className="bg-surface rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 pb-6 border-b border-border">
                <div>
                  <p className="text-sm text-fg-muted">Claim Number</p>
                  <p className="text-lg font-semibold text-fg">{selectedClaim.claimNumber}</p>
                </div>
                <div>
              ...
              {/* Claim Details */}
              <div>
                <h3 className="text-lg font-semibold text-fg mb-4">Claim Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-fg-muted">Subject</p>
                    <p className="text-fg font-semibold">{selectedClaim.subject}</p>
                  </div>
                  <div>
                    <p className="text-sm text-fg-muted">Date Submitted</p>
                    <p className="text-fg font-semibold">{formatDate(selectedClaim.createdAt)}</p>
                  </div>
                </div>
                <div className="mt-4">
                  <p className="text-sm text-fg-muted">Description</p>
                  <p className="text-fg mt-2 whitespace-pre-wrap">{selectedClaim.description}</p>
                </div>
              </div>
              ...
              {(selectedClaim.productName || selectedClaim.productSku || selectedClaim.installDate || selectedClaim.issueDate) && (
                <div className="border-t border-border pt-6">
                  <h3 className="text-lg font-semibold text-fg mb-4">Product Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedClaim.productName && (
                      <div>
                        <p className="text-sm text-fg-muted">Product Name</p>
                        <p className="text-fg font-semibold">{selectedClaim.productName}</p>
                      </div>
                    )}
                    {selectedClaim.productSku && (
                      <div>
                        <p className="text-sm text-fg-muted">Product SKU</p>
                        <p className="text-fg font-semibold">{selectedClaim.productSku}</p>
                      </div>
                    )}
                    {selectedClaim.installDate && (
                      <div>
                        <p className="text-sm text-fg-muted">Install Date</p>
                        <p className="text-fg font-semibold">{formatDate(selectedClaim.installDate)}</p>
                      </div>
                    )}
                    {selectedClaim.issueDate && (
                      <div>
                        <p className="text-sm text-fg-muted">Issue Date</p>
                        <p className="text-fg font-semibold">{formatDate(selectedClaim.issueDate)}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedClaim.contactName && (
                      <div>
                        <p className="text-sm text-fg-muted">Contact Name</p>
                        <p className="text-fg font-semibold">{selectedClaim.contactName}</p>
                      </div>
                    )}
                    {selectedClaim.contactEmail && (
                      <div>
                        <p className="text-sm text-fg-muted">Email</p>
                        <p className="text-fg font-semibold">{selectedClaim.contactEmail}</p>
                      </div>
                    )}
                    {selectedClaim.contactPhone && (
                      <div>
                        <p className="text-sm text-fg-muted">Phone</p>
                        <p className="text-fg font-semibold">{selectedClaim.contactPhone}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {(selectedClaim.siteAddress || selectedClaim.siteCity || selectedClaim.siteState || selectedClaim.siteZip) && (
                <div className="border-t border-border pt-6">
                  <h3 className="text-lg font-semibold text-fg mb-4">Site Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedClaim.siteAddress && (
                      <div>
                        <p className="text-sm text-fg-muted">Address</p>
                        <p className="text-fg font-semibold">{selectedClaim.siteAddress}</p>
                      </div>
                    )}
                    {selectedClaim.siteCity && (
                      <div>
                        <p className="text-sm text-fg-muted">City</p>
                        <p className="text-fg font-semibold">{selectedClaim.siteCity}</p>
                      </div>
                    )}
                    {selectedClaim.siteState && (
                      <div>
                        <p className="text-sm text-fg-muted">State</p>
                        <p className="text-fg font-semibold">{selectedClaim.siteState}</p>
                      </div>
                    )}
                    {selectedClaim.siteZip && (
                      <div>
                        <p className="text-sm text-fg-muted">ZIP Code</p>
                        <p className="text-fg font-semibold">{selectedClaim.siteZip}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {selectedClaim.resolutionDetails && (
                <div className="border-t border-border pt-6 bg-green-50 p-4 rounded-lg">
      ...
      {showNewClaimForm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-surface rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-border-strong rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                >
              ...
              <div className="border-t border-border pt-4">
                <h3 className="text-sm font-semibold text-fg mb-4">Product Information (Optional)</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                    type="text"
                    ...
                    className="px-4 py-2 border border-border-strong rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                  <input
                    type="text"
              ...
              {/* Contact Information */}
              <div className="border-t border-border pt-4">
                <h3 className="text-sm font-semibold text-fg mb-4">Contact Information (Optional)</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                    type="text"
              ...
              {/* Site Information */}
              <div className="border-t border-border pt-4">
                <h3 className="text-sm font-semibold text-fg mb-4">Site Information (Optional)</h3>
            ...
            <div className="sticky bottom-0 bg-surface-muted border-t border-border px-6 py-4 flex gap-3">
              <button
                onClick={() => setShowNewClaimForm(false)}
                className="flex-1 px-4 py-2 border border-border-strong rounded-lg font-semibold text-fg-muted hover:bg-surface-muted transition-colors"
              >
                Cancel
              </button>
              <button
