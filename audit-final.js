...
function normalizeProduct(row) {
  // Get price column - try multiple possible names
  const basePrice = parseFloat(
    row["Base Price"] || row["basePrice"] || row["Price"] || row["Retail Price"] || 0
  ) || 0;
  ...
  return {
    ...
    category: row["Clean Category"] || row["Category"] || row["Original InFlow Category"] || "",
    subcategory: row["Product Type"] || row["SubCategory"] || "",
    basePrice: basePrice,
    cost: parseFloat(row["Unit Cost"] || row["Cost"] || row["cost"] || 0) || 0,
    doorSize: row["Door Size"] || row["doorSize"] || "",
    handing: row["Handing"] || row["handing"] || "",
    coreType: row["Core Type"] || row["coreType"] || "",
    panelStyle: row["Panel Style"] || row["panelStyle"] || "",
    jambSize: row["Jamb Size"] || row["jambSize"] || "",
    casingCode: row["Casing"] || row["casingCode"] || "",
    hardwareFinish: row["Hardware Finish"] || row["hardwareFinish"] || "",
    material: row["Material"] || row["material"] || "",
    fireRating: row["Fire Rating"] || row["fireRating"] || "",
  };
}
...
function auditProducts() {
  ...
  console.log(`📁 2. PRODUCTS PER CATEGORY:`);
  ...
  sortedCategories.forEach(([category, count]) => {
    ...
    console.log(`   ${category.substring(0, 40).padEnd(40)}: ${String(count).padStart(5)} (${String(percentage).padStart(5)}%)`);
  });
  ...
  if (negativeMargin.length > 0) {
    ...
    negativeMargin.slice(0, 5).forEach(({ name, cost, basePrice }) => {
      ...
      console.log(
        `      ${name.substring(0, 50)}: cost=$${cost.toFixed(2)}, price=$${basePrice.toFixed(2)} (${margin}% margin)`
      );
    });
  }
  ...
  Object.entries(categoryPrices)
    ...
    .forEach(([category, { sum, count }]) => {
      ...
      console.log(`      ${category.substring(0, 35).padEnd(35)}: $${String(avg).padStart(10)} (${count} w/ price)`);
    });
  ...
  if (longNames.length > 0) {
    ...
    longNames.slice(0, 5).forEach(({ name, length }) => {
      const truncated =
        name.length > 77 ? name.substring(0, 74) + "..." : name;
  ...
  distinctCategories.slice(0, 20).forEach((cat) => {
    const catCount = categoryCounts[cat] || 0;
    console.log(`      • ${cat} (${catCount})`);
  });

  if (distinctCategories.length > 20) {
    console.log(`      ... and ${distinctCategories.length - 20} more`);
  }
