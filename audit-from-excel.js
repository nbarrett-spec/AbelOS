...
const LIVE_FILE = "/sessions/jolly-happy-carson/mnt/Abel Lumber/Abel_Product_Catalog_LIVE.xlsx";
...
function readExcelSheet(filePath, sheetName) {
  ...
  if (!ws) {
    ...
    console.log(`Sheet "${sheetName}" not found. Available sheets: ${available.join(", ")}`);
    ws = wb.Sheets[available[0]];
  }
...
function normalizeProduct(row) {
  ...
  return {
    ...
    basePrice: parseFloat(row["BasePrice"] || row["Base Price"] || row["Price"] || 0) || 0,
    cost: parseFloat(row["Cost"] || row["cost"] || 0) || 0,
    doorSize: row["DoorSize"] || row["Door Size"] || row["doorSize"] || "",
    handing: row["Handing"] || row["handing"] || "",
    coreType: row["CoreType"] || row["Core Type"] || row["coreType"] || "",
    panelStyle: row["PanelStyle"] || row["Panel Style"] || row["panelStyle"] || "",
    jambSize: row["JambSize"] || row["Jamb Size"] || row["jambSize"] || "",
    casingCode: row["CasingCode"] || row["Casing Code"] || row["casingCode"] || "",
    hardwareFinish: row["HardwareFinish"] || row["Hardware Finish"] || row["hardwareFinish"] || "",
    material: row["Material"] || row["material"] || "",
    fireRating: row["FireRating"] || row["Fire Rating"] || row["fireRating"] || "",
  };
}
...
function auditProducts() {
  ...
  let allProducts = readExcelSheet(CATALOG_FILE, "Products");

  if (allProducts.length === 0) {
    console.log(`Trying alternate sheet names...`);
    const catalogWb = XLSX.readFile(CATALOG_FILE);
    console.log(`Available sheets: ${catalogWb.SheetNames.join(", ")}`);
    allProducts = readExcelSheet(CATALOG_FILE, catalogWb.SheetNames[0]);
  }
